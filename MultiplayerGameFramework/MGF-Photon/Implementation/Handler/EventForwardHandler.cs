...
namespace MGF_Photon.Implementation.Handler
{
    public class EventForwardHandler : ServerHandler, IDefaultEventHandler<IServerPeer>
    {
        ...
        public EventForwardHandler(ILogger log, IConnectionCollection<IClientPeer> connectionCollection,
        ...
        protected override bool OnHandleMessage(IMessage message, IServerPeer serverPeer)
        {
            ...
            {
                Log.DebugFormat("Looking for per Id {0}", new Guid((Byte[])message.Parameters[_serverConfiguration.PeerIdCode]));
                PhotonClientPeer peer = _connectionCollection.GetPeers<PhotonClientPeer>().FirstOrDefault(p =>
