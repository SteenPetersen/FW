...
namespace MGF.Photon.Implementation.Server
{
    public class TypeCache
    {
        ...
        public static byte[] SerializePeerInfo(object obj)
        {
            ...
            IEnumerable<byte> retVal = BitConverter.GetBytes(info.MasterEndPoint.Port).Concat(BitConverter
                .GetBytes(info.ConnectRetryIntervalSeconds)).Concat(BitConverter.GetBytes(info.IsSiblingConnection))
                .Concat(BitConverter.GetBytes(info.MaxTries)).Concat(BitConverter.GetBytes(info.NumTries))
